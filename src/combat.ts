  ...
  const exhaust = G.viewer.entities.add({
    position: new Cesium.CallbackPositionProperty(() => {
      ...
      return Cesium.Cartesian3.fromDegrees(
        p.lon + Math.sin(back) * m,
        p.lat + Math.cos(back) * m * 0.85,
        p.alt - 0.5,
      );
    }, false),
    point: {
      pixelSize: 18,
      color: Cesium.Color.fromCssColorString("#fb923c").withAlpha(0.95),
      outlineColor: Cesium.Color.fromCssColorString("#7dd3fc"),
      outlineWidth: 2,
      disableDepthTestDistance: Number.POSITIVE_INFINITY,
      scaleByDistance: new Cesium.NearFarScalar(60, 1.8, 14000, 0.12),
    },
  });
  return { jet, exhaust };
}
...
export function killEnemy(e: Enemy): void {
  ...
  e.fallT = 2.6;
  e.pitch = -0.35;
  const wreck = e.entity as { model?: { color?: unknown; colorBlendAmount?: number } } | null;
  if (wreck?.model) {
    try {
      wreck.model.color = Cesium.Color.fromCssColorString("#1e293b");
      wreck.model.colorBlendAmount = 0.7;
    } catch {
      /* */
    }
  }
...
function updateLock(dt: number): void {
  ...
  for (const e of G.enemies) {
    ...
    const cone = G.lockCone || LOCK_CONE;
    if (Math.abs(dh) < cone && d < best) {
      best = d;
      cand = e;
    }
  }
  if (cand) {
    ...
    const cone = G.lockCone || LOCK_CONE;
    const center = 1 - Math.min(1, Math.abs(dh) / cone);
    const range = 1 - Math.min(1, best / 3400);
    const prev = G.lockProg;
    G.lockProg = Math.min(1, G.lockProg + (0.85 + center * 1.7 + range * 0.9) * dt);
    if (prev < 0.12 && G.lockProg >= 0.12) G.lockAttempts += 1;
    if (prev < HARD_LOCK && G.lockProg >= HARD_LOCK) {
      audio.lock();
      G.locks += 1;
    }
  } else {
...
export function updateCombat(dt: number): void {
  ...
  for (const e of G.enemies) {
    if (!e.dead) updateEnemy(e, dt, { gun: fireEnemyGun, missile: fireEnemyMissile });
      ...
      e.fallT -= dt;
      e.alt = Math.max(40, e.alt - 95 * dt);
      e.pitch = Math.min(0.9, e.pitch + 0.45 * dt);
      e.roll += 1.1 * dt;
      e.heading += 0.35 * dt;
      if (e.fallT <= 0) {
        try {
