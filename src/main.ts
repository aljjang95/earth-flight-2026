...
async function runPrologue(): Promise<void> {
  ...
  if (new URLSearchParams(location.search).has("qa") || G.save.theatersCleared.length > 0) {
    await startMission();
    await audio.start();
    return;
  }
...
function wireUi(): void {
  ...
  const qSel = $("qualitySelect") as HTMLSelectElement | null;
  qSel?.addEventListener("change", () => {
    const v = qSel.value;
    G.autoQuality = v === "auto";
    if (v === "high" || v === "medium" || v === "low") applyQuality(v);
  });
