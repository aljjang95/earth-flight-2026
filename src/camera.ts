...
export function updateCamera(): void {
  ...
  if (G.introLook > 0 && G.cam !== "first") {
    ...
    cam.setView({
      destination: dest,
      orientation: { heading: p.heading, pitch, roll: 0 },
    });
  ...
  if (G.cam === "first") {
    ...
    const dir = Cesium.Matrix4.multiplyByPointAsVector(
      frame,
      new Cesium.Cartesian3(1, 0, G.mode === "free" ? -0.28 : -0.2),
    ...
    const camPitch = Math.max(-0.62, Math.min(-0.18, p.pitch * 0.12 - 0.46));
    const frame = Cesium.Transforms.headingPitchRollToFixedFrame(
      pos,
      new Cesium.HeadingPitchRoll(p.heading, camPitch, camRoll),
    );
    const dist = G.mode === "free" ? 115 : 132;
    const height = G.mode === "free" ? 44 : 50;
    const eye = Cesium.Matrix4.multiplyByPoint(frame, new Cesium.Cartesian3(-dist, 5, height), new Cesium.Cartesian3());
    const lookAt = Cesium.Matrix4.multiplyByPoint(frame, new Cesium.Cartesian3(32, 0, -34), new Cesium.Cartesian3());
